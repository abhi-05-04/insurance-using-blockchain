...
export default function Index() {
    ...
    const getQueue = async (id) => {
        ...
        const memb = await tmpInsurance.methods.getClaimList(2).call();
        console.log(await tmpInsurance.methods.getClaimList(2).call())
        let items = [];
        var k = 0;
        if (memb == undefined) {
            var x = 0;
                    ...
                    description: (
                        <Card.Content extra>
                            <div className='buttons'>
                                <Button basic color='black' href="/admin/completeclaim/details" onClick={()=>setdata(memb[i])} style={{ margin: 3 }}>
                                    Show Details
                                </Button>
                            </div>
                        </Card.Content>
